...
class NavigationAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    private var commonStatusBarFrame: CGRect? {
    ...
    private func popAnimation(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let snapshootHeight = (commonStatusBarFrame?.size.height ?? 0) + (fromViewController?.navigationController?.navigationBar.frame.size.height)!

        let fakeBar = fromViewController?.navigationController?.view.resizableSnapshotView(
            from: CGRect(x: 0,
                         y: 0,
                         width: (fromViewController?.view.frame.size.width)!,
                         height: snapshootHeight),
